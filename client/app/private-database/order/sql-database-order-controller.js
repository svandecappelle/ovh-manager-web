    ...
    class SqlDatabaseOrderCtrl {
        ...
        getAvailableOrderCapacities () {
            ...
            return this.privateDatabaseService
                ...
                .then((res) => this.$q
                    .all(
                            _.map(res.hostings, (domainName) => this.hostingService.getHosting(domainName))
                        )
                    .then((domains) => {
        ...
        getDurationsstart (data) {
            const hosting = this.model.hosting;
        ...
        getPricespremium (data, durations) {
            ...
            return this.$q
                .all(
                    _.map(durations, (duration) => this.privateDatabaseService.orderPrice(version, ram, duration).then((details) => {
        ...
        getPricesstart (data, durations) {
            ...
            return this.$q
                .all(
                    _.map(durations, (duration) => this.hostingOptionOrderService.getSqlPersoPrice(hosting, startDbVersion, duration).then((details) => {
                        _.find(data.durations, "duration", duration).details = details;
                        return details;
                    }))
                )
